/**
 * Unit tests for entropy-calc.js
...
describe('calculateBruteforce', () => {
  ...
  test('handles null/undefined gracefully', () => {
    expect(calculateBruteforce(null)).toBe(0);
    expect(calculateBruteforce(undefined)).toBe(0);
  });
});
...
describe('ATTACK_PROFILES', () => {
  test('contains all expected profiles', () => {
    const expectedProfiles = ['online', 'paranoid', 'strong', 'standard', 'weak', 'legacy'];
  ...
  test('all profiles have required properties', () => {
    Object.values(ATTACK_PROFILES).forEach(profile => {
  ...
  test('profiles have expected speed order', () => {
    ...
    expect(ATTACK_PROFILES.online.speed).toBeLessThan(ATTACK_PROFILES.paranoid.speed);
    expect(ATTACK_PROFILES.paranoid.speed).toBeLessThan(ATTACK_PROFILES.strong.speed);
    expect(ATTACK_PROFILES.strong.speed).toBeLessThan(ATTACK_PROFILES.standard.speed);
    expect(ATTACK_PROFILES.standard.speed).toBeLessThan(ATTACK_PROFILES.weak.speed);
    expect(ATTACK_PROFILES.weak.speed).toBeLessThan(ATTACK_PROFILES.legacy.speed);
  });
...
describe('Integration: Example passphrase', () => {
  test('istuvillaan.R8U.pergola.lastain has ~68 bits and is rated "strong"', () => {
    ...
    expect(checkNISTCompliance(passphrase.length, false).compliant).toBe(true);
  });
});