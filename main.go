...
func main() {
	...
	if settings.MaxLength > 0 {
		smallWords = entropy.WordlistSubset(settings.MaxLength, internal.Words)
	}

	passphrase, err := internal.GeneratePassword(settings.WordCount, settings.MaxLength, settings.Delimiter, internal.Words)
